...
Army generate_random_army() {
	...
	std::generate_n(std::back_inserter(*army.army_), army_size_max_, [&army]() { return std::make_shared<Creature>(creature::get_ptr_to_creature_template_from_database(static_cast<CreatureTemplateID>((
		get_random_number(static_cast<int>(CreatureTemplateID::kHumanSpearman), static_cast<int>(CreatureTemplateID::kCreatureTemplateMax) - 1)))), army.generate_creature_complex_id());});
