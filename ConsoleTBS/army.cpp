...
Army generate_random_army() {
	...
	std::generate_n(std::back_inserter(*army.army_), army_size_max_, []() { return std::make_shared<Creature>(creature_database_get_templates(static_cast<CreatureTemplate>((
	...
	std::for_each(std::execution::par_unseq, army.army_->begin(), army.army_->end(), [=](std::shared_ptr<Creature> creature) {creature->join_army(army_id);});
...
void Army::kill_creature(size_t dead_creature_id){
	auto dead_creature_iter = std::find_if(std::execution::par_unseq, army_->begin(), army_->end(), [dead_creature_id](std::shared_ptr<Creature> creature) { return creature->get_creature_id() == dead_creature_id; }); // there are unique creature_id for every creature, so there can be par_unsec execution
